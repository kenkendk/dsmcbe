...
void readimage_grey(char* filename, void* allocator(size_t), struct IMAGE_FORMAT_GREY* imageinfo)
{
	...
	imageinfo->image = (unsigned char*)allocator(imageinfo->width * imageinfo->height);
	...
	for(y = 0; y < imageinfo->height; y++)
	{
		for(x = 0; x < imageinfo->width; x++)
		{
			...
			//Convert the RGB values into grayscale
			imageinfo->image[(y*imageinfo->width)+x] = (buf[0] + buf[1] + buf[2]) / 3;
		}
...
void writeimage_grey(char* filename, struct IMAGE_FORMAT_GREY* imageinfo)
{
	...
	for(y = 0; y < imageinfo->height; y++)
		for(x = 0; x < imageinfo->width; x++)
		{
			buf[0] = imageinfo->image[x+(y*imageinfo->width)];
			buf[1] = imageinfo->image[x+(y*imageinfo->width)];
			buf[2] = imageinfo->image[x+(y*imageinfo->width)];
