...
int FoldPrototein(unsigned long long id)
{
    ...
    StartDMAReadTransfer(prototein, ea, prototein_buffer_length, 3);
    ...
    while(ea != 0)
    {
	    ...
	    /*printf("memdump: ");
		for(i = 80; i < 120; i++)
			printf("%x ", ((unsigned char*)current_buffer)[i]);
		printf("\n");*/
		...
		/*for(i = 0; i < 4; i++)
			printmap(&queue[((*work).item_length*i)], (*work).item_length, 1);*/
	    ...
	    for(i = 0; i < (*work).worksize; i++)
	    {
			...
			/*if (((i % 20) == 0) || (i >= 220))
			{
				...
				printmap(places, (*work).item_length, 1);
			}*/
    	...
    	//printmap(winner, prototein_length, 1);
		WaitForDMATransferByGroup(4);
        StartDMAWriteTransfer(winner, ea_winner, winner_buffer_length, 4);
